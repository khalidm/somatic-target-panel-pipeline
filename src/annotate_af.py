...
def main(sample, vcf_fn):
  ...
  vcf_in.add_info_to_header({'ID': 'VAF', 'Description': 'Calculated allele frequency', 'Type':'Float', 'Number': '1'})
  #vcf_in.add_info_to_header({'ID': 'DP_N', 'Description': 'Read depth for tier1 in Normal sample', 'Type':'Integer', 'Number': '1'})
  #vcf_in.add_info_to_header({'ID': 'DP_T', 'Description': 'Read depth for tier1 in Tumor sample', 'Type':'Integer', 'Number': '1'})
  ...
  for variant_count, variant in enumerate(vcf_in):
    ...
    variant.INFO["VAF"] = af
    #variant.INFO["DP_N"] = af
    #variant.INFO["DP_T"] = af
